// Shared primitives for the portfolio hi-fi.
// All components reference CSS variables defined in the host HTML so the
// Tweaks panel can re-theme them at runtime.

const { useState, useEffect, useRef } = React;

// ─── Photo slideshow ─────────────────────────────────────────
// Rotating placeholder. Real photos slot in via the `images` prop.
const PhotoSlideshow = ({ size = 240, images = [], labels = [], rounded = '50%', interval = 3200 }) => {
      ...
      {slides.map((s, n) => {
        ...
        return (
          <div key={n} style={{ ...{
              ...
              background: isImage ? `url(${s}) center/cover` : 'linear-gradient(135deg, var(--accent-soft) 0%, #f3e6df 60%, #e8d5cb 100%)',
              display: 'flex', alignItems: 'flex-end', justifyContent: 'center',
              color: 'var(--brown-700)', fontFamily: 'DM Sans, sans-serif', fontSize: 11, letterSpacing: 0.5,
...
// ─── LinkedIn button ─────────────────────────────────────────
const LinkedInBtn = ({ size = 'md', variant = 'solid', label = 'Connect on LinkedIn' }) => {
  const sizes = {
  ...
  return (
    <a href="#" style={{
      ...
      transition: 'all 0.25s ease', borderRadius: "5px"
    }}
    onMouseEnter={(e) => {
      e.currentTarget.style.background = isOutline ? 'var(--brown-900)' : 'var(--brown-700)';
...
// ─── GitHub button ─────────────────────────────────
const GitHubBtn = ({ size = 'md', variant = 'solid', label = 'GitHub', href = '#' }) => {
  const sizes = {
    sm: { fs: 11, py: 8, px: 14, ic: 13 },
    md: { fs: 13, py: 12, px: 20, ic: 15 },
    lg: { fs: 14, py: 16, px: 26, ic: 17 }
...
// ─── Award badge ─────────────────────────────────────────────
const AwardBadge = ({ name, detail, year }) =>
<div style={{
...
const AwardStrip = ({ awards, mobile }) =>
<div style={{
...
// ─── Image placeholder ───────────────────────────────────────
const ImgPlaceholder = ({ height = 220, label = 'project image', src = null, style = {} }) => {
  if (src) {
    return <div style={{ height, background: `url(${src}) center/cover`, ...style }} />;
  }
  return (
    <div style={{
      height,
...
const HorizontalTimeline = ({ timeline, animate = true }) => {
  const minY = 2019;
  const maxY = 2026.5;
            ...
            <React.Fragment key={i}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  color: 'var(--brown-900)', letterSpacing: -0.3, fontSize: "14px"
                }}>{t.role}</div>
                <div style={{
                  fontSize: 10,
                  letterSpacing: 2, textTransform: 'uppercase',
                  color: 'var(--brown-500)', marginTop: 4, fontFamily: "\"DM Sans\""
                }}>{t.company}</div>
              </div>
              {/* year below */}
              <div style={{
                ...
                fontFamily: 'DM Sans, sans-serif', fontSize: 11,
                color: 'var(--brown-500)', letterSpacing: 1,
                opacity: segOpacity
              }}>{t.start}</div>
            </React.Fragment>);
...
// ─── Project card ────────────────────────────────────────────
const ProjectCard = ({ project, onOpen, layout = 'grid', titleScale = 1 }) => {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16 }}>
        <div style={{ flex: 1 }}>
          <div style={{
            ...
            color: 'var(--brown-900)', letterSpacing: -1.5, borderStyle: "solid", borderWidth: "0px", fontSize: "50px", padding: "1px", margin: "0px"
          }}>{project.metric}</div>
          <div style={{
            fontFamily: 'DM Sans, sans-serif', fontSize: 11,
            letterSpacing: 1.5, textTransform: 'uppercase',
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <ImgPlaceholder height={400} label={project.imgLabel} src={project.image} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, marginBottom: 48 }}>
            <div>
              ...
              <p style={{
                ...
                lineHeight: 1.6, color: 'var(--brown-800)', marginTop: 14
              }}>{project.problem}</p>
            </div>
            <div>
...
const VerticalTimeline = ({ timeline, animate = true }) => {
  const [progress, setProgress] = useState(animate ? 0 : 1);
  const ref = useRef(null);
  ...
  return (
    <div ref={ref} style={{ position: 'relative', paddingLeft: 24 }}>
      ...
      {items.map((t, i) => {
        const itemThreshold = i / items.length;
        const visible = progress >= itemThreshold;
        ...
        return (
          <div key={i} style={{
            ...
            <div style={{
              fontFamily: 'DM Sans, sans-serif', fontSize: 10,
              letterSpacing: 1.5, textTransform: 'uppercase',
              color: t.current ? 'var(--accent)' : 'var(--brown-500)',
              marginBottom: 3
            }}>
              {t.start}{t.end === 'now' ? ' — Present' : ` — ${t.end}`}
            </div>
            <div style={{
              fontFamily: 'Cormorant Garamond, serif',
...
Object.assign(window, {
  PhotoSlideshow, LinkedInBtn, GitHubBtn, AwardBadge, AwardStrip, Eyebrow, ImgPlaceholder, Tag,
  HorizontalTimeline, VerticalTimeline, ProjectCard, ProjectModal
});